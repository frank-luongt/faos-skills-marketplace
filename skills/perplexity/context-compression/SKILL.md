...
description: "Advanced context compression strategies for long-running agent sessions. Covers anchored iterative summarization, opaque compression, regenerative full summary, trigger thresholds at 70-80% utilization, and probe-based evaluation with 6-dimension quality scoring. Use when: context compression, token budget, context window management, long sessions, summarize conversation history, reduce token usage."
tags: [context, compression, agents, optimization]
---
...
This skill connects to several related skills:

- `context-optimization` - Broader optimization strategies (compaction, masking, caching, partitioning)
- `multi-agent-patterns` - Compression relates to scratchpad and summary memory patterns
- `memory-systems` - Compression relates to persistent memory and retrieval
...
<!-- Source: .faos/custom/skills/ai-ml/context-compression/SKILL.md -->
