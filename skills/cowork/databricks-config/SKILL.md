...
- `/databricks-config https://adb-1234567890123456.7.azuredatabricks.net/` - Configure using
  workspace host URL
...
1. Determine the profile and host:
   - If a parameter is provided and it starts with `https://`, treat it as a workspace host:
     - Extract profile name from the host (e.g., `adb-1234567890123456.7.azuredatabricks.net` →
       `adb-1234567890123456`, `my-company-dev.cloud.databricks.com` → `my-company-dev`)
   ...
   - If no parameter is provided, ask the user which profile they want to configure (default:
     DEFAULT)
...
<!-- Source: .faos/custom/skills/cloud/databricks/databricks-config/SKILL.md -->
