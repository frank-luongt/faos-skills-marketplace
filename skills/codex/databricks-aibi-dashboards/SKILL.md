...
Create Databricks AI/BI dashboards (formerly Lakeview dashboards). **Follow these guidelines
strictly.**
...
**WARNING: If you deploy without testing queries, widgets WILL show "Invalid widget definition"
errors!**
...
> **CRITICAL: Field Name Matching Rule** The `name` in `query.fields` MUST exactly match the
> `fieldName` in `encodings`. If they don't match, the widget shows "no selected fields to
> visualize" error!
...
If you need conditional logic or multi-field formulas, compute a derived column in the dataset SQL
first.
...
- **CRITICAL: Multiple items in the `lines` array are concatenated on a single line, NOT displayed
  as separate lines!**
...
**Key Insight**: A filter only affects datasets that contain the filter field. To have a filter
affect only specific pages:
...
> **CRITICAL**: Do NOT use `associative_filter_predicate_group` - it causes SQL errors! Use a simple
> field expression instead.
...
6. **Field `name` in query.fields matches `fieldName` in encodings exactly** (e.g., both
   `"sum(spend)"`)
7. Counter datasets: use `disaggregated: true` for 1-row datasets, `disaggregated: false` with
   aggregation for multi-row
...
**This is a field name mismatch error.** The `name` in `query.fields` must exactly match the
`fieldName` in `encodings`.
...
- Check `widgetType` is one of: `filter-multi-select`, `filter-single-select`,
...
- **Global filters** (on `PAGE_TYPE_GLOBAL_FILTERS` page) affect all datasets containing the filter
  field
...
<!-- Source: .faos/custom/skills/cloud/databricks/databricks-aibi-dashboards/SKILL.md -->
